...
[**Report a vulnerability**](https://github.com/hassio-addons/addon-nginx-proxy-manager/security/advisories/new)
...
- Issues in Home Assistant, its Supervisor, or the operating system the add-on
  runs on; please report those to the
  [Home Assistant project](https://github.com/home-assistant/core/security/policy).
- Issues only reproducible on add-on versions older than the latest release.
- Exposure resulting from deliberately publishing the add-on's ports to an
  untrusted network without further protection. The add-on assumes the
  administrator controls how it is exposed.

## Scope

This security policy covers the Nginx Proxy Manager Home Assistant add-on
